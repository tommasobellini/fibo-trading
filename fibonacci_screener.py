...
if __name__ == "__main__":
    # Capitalizzazione minima (esempio: 10 miliardi di dollari)
    min_market_cap = 10_000_000_000

    # 1. Recupera la lista delle aziende S&P 500
    sp500_df = get_sp500_companies()

    if sp500_df.empty:
        print("Failed to retrieve the list of S&P 500 companies.")
    else:
        # 2. Estrae i ticker
        tickers = sp500_df['Symbol'].tolist()

        # 3. Sostituisce eventuali punti (.) con trattini (-) per compatibilità con yfinance
        tickers = [ticker.replace('.', '-') for ticker in tickers]

        # 4. Filtra i ticker in base alla capitalizzazione di mercato
        print("Filtering stocks by market capitalization...")
        filtered_tickers = filter_stocks_by_market_cap(tickers, min_market_cap)

        # 5. Esegue il Fibonacci retracement screener (61,8%) sui ticker filtrati
        #    e controlla Stocastico, RSI e MACD (oversold)
        print("Running Fibonacci retracement screener for last month data (61.8% level) + Stoch, RSI, MACD oversold...")
        df_results = fibonacci_retracement_screener(filtered_tickers)

        # 6. Mostra i risultati
        if not df_results.empty:
            pd.set_option('display.max_rows', None)  # Mostra tutte le righe
            print("Stocks near 61.8% Fibonacci retracement level (last month) AND oversold on RSI, Stoch, MACD:")
            print(df_results)
        else:
            print("No stocks meet the criteria (near 61.8% Fib AND oversold indicators).")
